...
const { visibleCat, visiblePreference, visibleMemo, mirrorMode, penetrable, alwaysOnTop } = storeToRefs(shortcutStore)
...
useKeyPress(visibleMemo, () => {
  if (!memoStore.enabled) return
  memoStore.toggle()
})
...
<template>
  <ProList :title="$t('pages.preference.shortcut.title')">
    ...
    <ProListItem
      :description="$t('pages.preference.shortcut.hints.toggleMemo')"
      :title="$t('pages.preference.shortcut.labels.toggleMemo')"
    >
      <Shortcut v-model="shortcutStore.visibleMemo" />
    </ProListItem>
